...
function handleWebSocketMessage(data) {
    ...
    switch (data.metadata.message_type) {
        ...
        case "session_reconnect":
            Logger.error(`Received session reconnect.`);
...
async function deleteChatMessage(e) {
...
async function onSendChatMessage(e) {
    await tryChatAction(e, sendChatMessage, "send chat message", `Chat: ${e.chatMessage}`);
}

async function onDeleteChatMessage(e) {
    await tryChatAction(e, deleteChatMessage, `delete chat message: ${e.message_id}`, `Deleted chat message: ${e.message_id}`);
}

async function onTimeoutUser(e) {
    await tryChatAction(e, timeoutUser, `timeout user: ${e.userId}`, `Timed out user: ${e.userId}`);
}

async function tryChatAction(e, action, actionText, successText) {
    const response = await action(e);
    if (response.status >= 400) {
        const data = await response.json();
        Logger.error(`Failed to ${actionText}.`);
        Logger.error(data);
        if (response.status >= 401)
            await refreshCurrentToken();
        const retryResponse = await action(e);
        if (retryResponse.status != 200) {
            const retryData = await retryResponse.json();
            Logger.error(`${actionText} failed. Giving up.`);
        }
    }
    else {
        Logger.log(successText);
    }
}
